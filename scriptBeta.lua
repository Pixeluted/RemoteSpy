    ...
    for i,v in next, myTable do
        ...
        elseif primType == "userdata" then
            ...
            if mainType == "Instance" then
                if not hasNilParentedInstance and not v.IsAncestorOf(v, game) then
...
userdataValue = function(data) -- FORKED FROM HYDROXIDE
...
local function addConnection(remote, signalType, signal)
    ...
    setThreadIdentity(8)
    local remoteId = getDebugId(remote)
    local remoteType = getproperties(remote).ClassName
